...
from app.retrieval.search_client import upload_chunks, ensure_index
...
def make_search_key(value: str) -> str:
    """
    Azure AI Search document keys only allow:
    - letters
    - numbers
    - underscore (_)
    - dash (-)
...
async def ingest(file: UploadFile = File(...)):
    ...
    for chunk in extracted["chunks"]:
        ...
        for i, (piece, vec) in enumerate(zip(pieces, vectors)):
            ...
            document_id = make_search_key(
                f"{file.filename}-{chunk['page']}-{i}"
            )
